...
INITIAL_OVERFLOW=$(kubectl exec "$FORTIO_POD" -c istio-proxy -n bookinfo -- curl -s localhost:15000/stats | { grep -E "cluster.outbound\|9080\|.*ratings.bookinfo.svc.cluster.local.*upstream_rq_pending_overflow" || true; } | cut -d ":" -f 2 | tr -d " " | awk '{s+=$1} END {print s}')
...
kubectl exec "$FORTIO_POD" -c fortio -n bookinfo -- fortio load -c 20 -qps 0 -n 100 -loglevel Warning http://ratings:9080/ratings/0 || true

echo "Fetching final pending overflow stats..."
FINAL_OVERFLOW=$(kubectl exec "$FORTIO_POD" -c istio-proxy -n bookinfo -- curl -s localhost:15000/stats | { grep -E "cluster.outbound\|9080\|.*ratings.bookinfo.svc.cluster.local.*upstream_rq_pending_overflow" || true; } | cut -d ":" -f 2 | tr -d " " | awk '{s+=$1} END {print s}')
